...
def DataProcessing(Csi_Mat_Path):
    ...
    amplitudes_and_phases = CsiToAmplitudeAndPhase.getAmplitudesAndPhases(Csi_Mat_Path)
    ...
    amplitudes = WeightedMovingAverage.weightMoveAverage(amplitudes, N)
    plotAmplitudes(amplitudes[:, 29, 0], 'Weight_moving_average_filter')

    for k in range(0, N):
        phases[k] = PhaseSanitization.PhaseSanitization(phases[k], 30, 6)
