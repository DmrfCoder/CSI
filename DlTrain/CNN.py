...
from DlTrain.TrainNet import classes
...
def conv2d(c_input, c_filter, c_strides):
    return tf.nn.conv2d(input=c_input, filter=c_filter, strides=c_strides, padding='SAME')


def max_pool_2x2(p_input, p_ksize, p_strides):
    return tf.nn.max_pool(value=p_input, ksize=p_ksize, strides=p_strides, padding='SAME')
...
def CNN(in_x):
    ...
    h_pool1 = max_pool_2x2(h_conv1, [2, 2, 1, 1], [2, 2, 1, 1])

    w_conv2 = cnn_weight_variable([5, 3, 6, 10])
    b_conv2 = cnn_bias_variable([10])
    h_conv2 = tf.nn.relu(conv2d(c_input=h_pool1, c_filter=w_conv2, c_strides=[3, 3, 1, 1]) + b_conv2)
